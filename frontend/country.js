...
async function renderComments() {
  ...
  cmtOffset = 0;
  let rows;
  try { rows = await api.getComments(TARGET_TYPE, countryId, CMT_PAGE, 0); } catch { rows = []; }
  if (!rows || rows.length === 0) {
...
async function loadMoreComments() {
  ...
  let rows;
  try { rows = await api.getComments(TARGET_TYPE, countryId, CMT_PAGE, cmtOffset); } catch { rows = []; }
  if (rows && rows.length) {
